...
'''
#g.link_prediction(data, data.va, zs, include_tr=False)
...
def new_link_prediction(data, partition_fn, zs, start=0, end=None, 
    ...
    for i in range(start, end):
        ...
        eis.append(ei)
        eis_tmp = torch.cat(eis,dim=1)

        a = b
        b = to_dense_adj(eis_tmp, max_num_nodes=data.num_nodes)[0].bool()
        ...
        p.append(new_links)
        p_tmp = torch.cat(p,dim=1)
        n.append(
            fast_negative_sampling(
                eis_tmp, p_tmp.size(1), data.num_nodes
            )
        )
